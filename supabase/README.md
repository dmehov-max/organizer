...
- `functions/client-messages/` — публична точка (без Auth), двупосочен
  чат с клиента (0061) — огледална на upload-document. Ползва се само
  от `upload.html` (клиентът чете/пише нишката); index.html пише
  директно през supabase-js (authenticated), не през тази функция.
  Виж README-то в папката — **след деплой запиши реалното име в
  `CLIENT_MESSAGES_URL` в `upload.html`.**
...
Всичките осем функции имат `index.test.ts` до себе си — тестват
